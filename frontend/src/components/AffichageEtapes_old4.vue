  ...
  <div class="max-w-6xl mx-auto">
    ...
    <div class="bg-white dark:bg-gray-900 rounded-2xl border border-stone-200 dark:border-gray-800 p-5 shadow-sm mb-5">
      ...
      <div class="flex gap-1 mb-2.5">
        <button v-for="(e, i) in etapes" :key="i" @click="allerA(i)"
          ...
          :class="[
            ...
            e.type === 'INITIALISATION' ? 'bg-indigo-400' : e.type === 'DEGENERESCENCE' ? 'bg-amber-400' : 'bg-violet-400'
          ]"
      ...
      <div class="flex flex-wrap gap-4 text-xs text-gray-400 dark:text-gray-500">
        ...
        <span v-if="nbDegen > 0" class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block"></span>Dégén. ({{ nbDegen }})</span>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-5 mb-5">
      ...
      <Transition name="slide" mode="out-in">
        <div :key="etapeIndex" class="min-h-0">
          ...
          <!-- Étape DEGENERESCENCE -->
          <PanneauDegenerescence
            v-else-if="etapeIndex < etapes.length && etapeActuelle.type === 'DEGENERESCENCE'"
            :etape="etapeActuelle"
            :config="config"
          />
        ...
        <Transition name="tab" mode="out-in">
          <div :key="onglet + '-' + etapeIndex" class="flex-1 overflow-auto">
            <MatriceFlux v-if="onglet === 'matrice'"
              ...
              :potentiels="potentielsActuels" :cases-epsilon="casesEpsilonActuelles" />
            <GrapheFlux v-else
              :flux="fluxCourant" :metadata="metadata" :config="config"
              :case-active="caseActiveActuelle" :cycle="cycleActuel"
              :potentiels="potentielsActuels" :is-dark="isDark" />
          </div>
        </Transition>
      </div>
    </div>
...
import MatriceFlux from './MatriceFluxold.vue'
import GrapheFlux from './GrapheFlux_old3.vue'
import PanneauOptimisation from './PanneauOptimisation.vue'
import PanneauDegenerescence from './PanneauDegenerescence_old.vue'

const props = defineProps({ etapes: Array, config: Object, metadata: Object, final: Object, isDark: Boolean })
...
const nbDegen = computed(() => props.etapes.filter(e => e.type === 'DEGENERESCENCE').length)
