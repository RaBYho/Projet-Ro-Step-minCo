<template>
  <div class="w-full">
    ...
    <div class="flex items-center gap-2 mb-3 flex-wrap">
      <span class="text-[10px] uppercase tracking-wider text-gray-400 dark:text-gray-500">Nœuds</span>
      <div
      ...
      >
        ...
        <template v-for="lien in liensVisibles" :key="`lf-${lien.i}-${lien.j}`">
          <text
            ...
            stroke-width="3" paint-order="stroke" stroke-linejoin="round"
            :font-size="fontLabel" font-weight="700" class="select-none">
              {{ valAr(lien) }}
          </text>
        </template>
        ...
        <g v-for="(src, i) in metadata.sources" :key="`s-${i}`">
          ...
          />
          <title>Source {{ i + 1 }} · offre {{ config?.offres?.[i] ?? '—' }}</title>
          <text :x="SX" :y="ySrc(i) - nodeLabelOffset"
            text-anchor="middle" dominant-baseline="middle"
            :fill="srcActif(i) ? '#ffffff' : srcAEpsilon(i) ? '#92400e' : '#6366f1'"
            :font-size="fontTag" font-weight="700" class="select-none">
            U{{ i + 1 }}
          </text>
          <text :x="SX" :y="ySrc(i) + nodeValueOffset"
            text-anchor="middle" dominant-baseline="middle"
            :fill="srcAEpsilon(i) ? '#d97706' : srcActif(i) ? '#e0e7ff' : (mode === 'potentiels' ? '#4f46e5' : '#6b7280')"
            :font-size="fontValue" font-weight="700" class="select-none">
            {{ valSrc(i) }}
          </text>
        </g>
        ...
        <g v-for="(dst, j) in metadata.destinations" :key="`d-${j}`">
          ...
          <circle
            ...
            class="transition-all duration-300"
          />
          <title>Destination {{ j + 1 }} · demande {{ config?.demandes?.[j] ?? '—' }}</title>
          <text :x="DX" :y="yDst(j) - nodeLabelOffset"
            text-anchor="middle" dominant-baseline="middle"
            :fill="dstActif(j) ? '#ffffff' : dstAEpsilon(j) ? '#92400e' : '#0d9488'"
            :font-size="fontTag" font-weight="700" class="select-none">
            C{{ j + 1 }}
          </text>
          <text :x="DX" :y="yDst(j) + nodeValueOffset"
            text-anchor="middle" dominant-baseline="middle"
    ...
    <div v-if="mode === 'potentiels' && !aPotentiels"
      ...
      :class="isDark ? 'bg-amber-900/20 border-amber-800 text-amber-400' : 'bg-amber-50 border-amber-200 text-amber-600'">
    ...
    <div
      ...
      :class="isDark ? 'text-gray-500 border-gray-800' : 'text-gray-400 border-stone-200'"
    >
      <span class="flex items-center gap-1">
        <span class="inline-block w-4 h-0.5 bg-indigo-300 rounded"></span>Actif
      </span>
      <span class="flex items-center gap-1">
        <span class="inline-block w-4 h-0.5 border-t border-dashed border-stone-400 dark:border-gray-600"></span>Nul
      </span>
...
const H = computed(() => PAD_TOP + PAD_BOT + (maxN.value - 1) * STEP.value + R.value * 2)
...
function srcAEpsilon(i) { return (props.casesEpsilon ?? []).some(([r]) => r === i) }
function dstAEpsilon(j) { return (props.casesEpsilon ?? []).some(([, c]) => c === j) }
